class SeedPhrase {
  ...
  /// Génère une nouvelle seed phrase de 12 mots
  static SeedPhrase generate12Words() {
    // Liste des mots BIP39 (version simplifiée pour la démo)
    final wordList = [
      ...
      'near', 'neck', 'need', 'negative', 'neglect', 'neighbor', 'nephew', 'nerve',
    ...
    for (int i = 0; i < 12; i++) {
      final index = (random + i * 17) % wordList.length;
  ...
  /// Génère une nouvelle seed phrase de 24 mots
  static SeedPhrase generate24Words() {
    final seedPhrase12 = generate12Words();
    final additionalWords = generate12Words().words;
    
    return SeedPhrase(
      words: [...seedPhrase12.words, ...additionalWords],
      createdAt: DateTime.now(),
    );
  }

  /// Convertit la seed phrase en chaîne de caractères
  String get phrase => words.join(' ');

  /// Vérifie si la seed phrase est valide (12 ou 24 mots)
  bool get isValid => words.length == 12 || words.length == 24;
  ...
  factory SeedPhrase.fromJson(Map<String, dynamic> json) {
    return SeedPhrase(
  ...
  @override
  String toString() {
    return 'SeedPhrase(words: ${words.length} mots, créée: $createdAt, vérifiée: $isVerified, sauvegardée: $isBackedUp)';
  }

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is SeedPhrase &&
        other.words.length == words.length &&
        other.words.every((word) => words.contains(word)) &&
